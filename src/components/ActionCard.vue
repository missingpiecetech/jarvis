<template>
  <q-card class="action-card q-mb-sm" :class="actionTypeClass">
    <q-card-section class="row items-center">
      <q-icon :name="actionIcon" :color="actionColor" size="24px" class="q-mr-sm" />
      <div class="col">
    ...
    <q-card-section v-if="action.matchingItems && action.matchingItems.length > 0" class="q-pt-none">
      <q-expansion-item
      ...
      >
        <div class="q-pa-sm">
          ...
          >
            <q-chip dense size="sm" :color="getPriorityColor(item.priority)" text-color="white">
              {{ item.priority }}
            </q-chip>
            {{ item.title }}
            <span v-if="item.dueDate" class="text-grey-6"> - Due: {{ formatDate(item.dueDate) }}</span>
          </div>
          <div v-if="action.matchingItems.length > 5" class="text-caption text-grey-6">
...
const actionTypeClass = computed(() => {
  const { action: verb } = props.action
  switch (verb) {
...
const actionDescription = computed(() => {
  return props.action.description || `${props.action.action} ${props.action.object}`
})
